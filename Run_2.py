...
class Run:
    ...
    def img_preprocess(self,image):
        ...
        #cv2.imshow('white_line_image',white_line_image)
        ...
        binary_image = cv2.adaptiveThreshold(closing, 255, cv2.ADAPTIVE_THRESH_MEAN_C, cv2.THRESH_BINARY, 11, 2)
        ...
        upper_white = np.array([255, 128, 255])
                   ...
                   # tflite 예측
                    ...
                    rstate == "go":
                        ...
                        if self.steering_angle >= 85 and self.steering_angle <= 95:
                            ...
                            if self.steering_angle >=80:
                                self.motor_right(self.speedSet,40)
                            elif self.steering_angle < 50:
                                self.motor_right(self.speedSet,self.speedSet)
                            elif self.steering_angle>15:
                                self.motor_right(self.speedSet,self.speedSet-20)
                            else:
                                self.motor_right(100,100)
                        elif self.steering_angle < 95:
                            if self.steering_angle >=80:
                                self.motor_left(self.speedSet,40)
                            elif self.steering_angle >125:
                                self.motor_left(self.speedSet,self.speedSet)
                            elif self.steering_angle >165:
                                self.motor_left(self.speedSet,self.speedSet-20)
                            else:
